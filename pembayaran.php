 			...
 			<?php
				if ($_GET['act'] == 'bayar' && $_GET['id']) {
					if (!empty($_POST)) {
						...
						extract($_POST);
						$q = mysql_query("insert into pembayaran Values(NULL,'$_GET[id]','$_SESSION[iam_user]','$gambar','$bayar','pending', 'belum dikirim', NOW())");
						if ($q) {
				...
				?>
 				<div class="row col-md-6">
 					<form action="" method="post" enctype="multipart/form-data">
 						...
 						<input type="number" class="form-control" name="bayar" required onchange="validasi_bayar()"><br>
 						<label>Bukti Pembayaran</label><br>
 						<input type="file" class="form-control" name="gambar" required><br>
 						<!-- <label>Bukti Pembayaran</label><br>
 						<textarea class="form-control" name="keterangan"></textarea><br /> -->
 						<input type="submit" onclick="return isValidForm()" name="form-input" value="Kirim" class="btn btn-success">
 ...
 <script>
 	function validasi_bayar() {
 		let totalbayar = <?= $total ?> + <?= $pesanan->ongkir ?>;
 		let input = $('input[name=bayar]')
 		let inputbayar = input.val()
 		if (inputbayar != totalbayar) {
 			input.addClass('has-error')
 			alert('Nominal harus sama dengan total tagihan\nTotal tagihan: ' + totalbayar)
 		} else {
