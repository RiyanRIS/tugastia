	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				if ($_GET['act'] == "terkirim") {
					extract($_GET);
					$updatePembayaran = mysql_query("update pembayaran set status_terkirim = 'dikirim' where id='$_GET[id]'");
					if ($updatePembayaran) {
						$qPembayaran = mysql_query("Select * from pembayaran where id_pesanan='$id_pesanan' and status='verified'") or die (mysql_error());
						$totalPembayaran = 0;
						while ($d = mysql_fetch_object($qPembayaran)) {
									...
									<td>
										<?php
											if ($dataPembayaran->status == "verified") {
												?>
												<a href="pembayaran.php?act=terkirim&id=<?php echo $dataPembayaran->id; ?>&id_pesanan=<?php echo $dataPembayaran->id_pesanan; ?>" class="btn btn-sm btn-primary">Terkirim</a>
												<?php
											}
										?>
									</td>
		...
		<?php while($data=mysql_fetch_object($q)){ ?> 
				<tr <?php if($data->status == "pending" ){ echo 'style="background:#cce9f8 !important;"'; } ?> > 
					...
					<td><?php echo $data->status_terkirim?></td> 
					<td>
